...
<div class="container-fluid py-4">
    ...
    <div class="row">
        <!-- Form thêm thông tin phiếu Chuyển -->
        <div class="col-md-6 mb-4">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">Thông Tin Phiếu Chuyển Kho</h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('QLphieuchuyenkho.store') }}" method="POST" id="formPhieuNhap">
                        @csrf
                        <div class="mb-3">
                            ...
                            @endif
                            @if (!isset($khoChuyen))
                                <select id="idKhoChuyen" name="idKhoChuyen" class="form-select" required>
                                    <option value="" selected disabled>-- Chọn Kho --</option>
                                    @foreach ($khoChuyens as $k)
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="NguoiChuyen" name="NguoiChuyen" class="form-control" required>
                        </div>
                        <div class="mb-3">
        ...
        <div class="col-md-6 mb-4">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                    <form action="{{ isset($sanpham) ? route('QLctthemchuyenkho.update', $key) : route('QLctthemchuyenkho.store') }}" method="POST">
                        @csrf
                        @if (isset($sanpham))
                            @method('PUT')
                        @endif
                        ...
                        @if(isset($sanpham))
                            <a href="{{ route('QLctthemchuyenkho.create') }}" class="btn btn-secondary">Hủy</a>
                        @endif
                    </form>
                </div>
            </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tbody>
                        @php
                            $chiTietTemp = session('chi_tiet_chuyen', []);
                        @endphp
                            ...
                            @foreach($chiTietTemp as $index => $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('QLctthemchuyenkho.edit', $item['idSanPham']) }}" class="btn btn-warning btn-sm">
                                            <i class="fas fa-edit"></i> Sửa
                                        </a>
                                        <form action="{{ route('QLctthemchuyenkho.destroy', $item['idSanPham']) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này?');">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="d-flex justify-content-end mb-4">
        ...
        <button type="submit" class="btn btn-primary" form="formPhieuNhap" 
            ...
            <i class="fas fa-save me-1"></i> Lưu Phiếu Chuyển Kho
        </button>
    </div>
</div>
<script>
