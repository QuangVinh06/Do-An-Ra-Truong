    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-4">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('QLphieunhapkho.update', $phieunhap->id) }}" method="POST" id="formPhieuNhap">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select id="idKho" name="idKho" class="form-select" required>
                                ...
                                @foreach ($kho as $k)
                                    <option value="{{ $k->id }}" {{ $phieunhap->idKho == $k->id ? 'selected' : '' }}>{{ $k->TenKho }}</option>
                                @endforeach  
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="NguoiGiaoHang" class="form-label fw-semibold">Người Giao Hàng</label>
                            <input type="text" id="NguoiGiaoHang" name="NguoiGiaoHang" class="form-control" value="{{ $phieunhap->NguoiGiaoHang }}" required>
                        </div>
                        <div class="mb-3">
        ...
        <div class="col-md-6 mb-4">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                    <form action="{{ isset($sanpham) ? route('QLchitietsuaphieunhap.update', $id) : route('QLchitietsuaphieunhap.store') }}" method="POST">
                        @csrf
                        @if (isset($sanpham))
                            @method('PUT')
                        @endif
                        <input type="hidden" name="idPhieuNhap" value="{{ $phieunhap->id }}">
                        <div class="mb-3">
                        ...
                        @if(isset($sanpham))
                            <a href="{{ route('QLchitietsuaphieunhap.show',$phieunhap->id) }}" class="btn btn-secondary">Hủy</a>
                        @endif
                    </form>
                </div>
            </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tbody>
                        @php
                            $chiTietTemp = session('sua_chi_tiet_nhap',[]);
                        @endphp
                            ...
                            @foreach($chiTietTemp as $index => $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('QLchitietsuaphieunhap.edit', $index) }}" class="btn btn-warning btn-sm">
                                            <i class="fas fa-edit"></i> Sửa
                                        </a>
                                        <form action="{{ route('QLchitietsuaphieunhap.destroy', $index) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này?');">
                                            @csrf
                                            @method('DELETE')
