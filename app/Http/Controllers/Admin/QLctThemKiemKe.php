...
class QLctThemKiemKe extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        foreach ($chiTietList as & $chiTiet) {
            if ($chiTiet['idSanPham'] == $data['idSanPham'] && $chiTiet['TrangThai'] == $data['TrangThai']) {
                ...
                session(['chi_tiet_kiemke' => $chiTietList]);
                return redirect()->route('QLphieukiemke.create',['kho'=>$data['idKho']])->with('success', 'Đã cập nhật số lượng sản phẩm trong danh sách');
            }
        }
        ...
        // Lưu lại vào session
        session(['chi_tiet_kiemke' => $chiTietList]);
        return redirect()->route('QLphieukiemke.create',['kho'=>$data['idKho']])->with('success', 'Đã thêm sản phẩm vào danh sách');
    }
    ...
    public function edit(string $id)
    {
        ...
        $sanp = ChiTietKho::where('idKho', $idKho)->pluck('idSanPham');
        $sps = SanPham::whereIn('id', $sanp)->get();
        return view('admin.QLkho.themphieukiemke', compact('sanpham','kho', 'sps','key'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        session(['chi_tiet_kiemke' => $chiTietList]);
        return redirect()->route('QLphieukiemke.create',['kho'=>$idKho])->with('success', 'Đã sửa sản phẩm mới thành công');
    ...
    public function destroy(string $id)
    {
        $chiTietList = session('chi_tiet_kiemke', []);
        unset($chiTietList[$id]);
        session(['chi_tiet_kiemke' => array_values($chiTietList)]);
        return redirect()->route('QLphieukiemke.create',['kho'=>session('idKho')])->with('success', 'Đã xóa sản phẩm khỏi danh sách');
    }
}
