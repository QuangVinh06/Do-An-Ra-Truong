...
class QLxuatkho extends Controller
{
    ...
    {
        $hopdong = HopDong::where('TrangThaiCoc','Đã thanh toán đặt cọc')->orwhere('TrangThaiCoc','Không yêu cầu cọc')->pluck('id');
        $donhangs = DonHang::whereIn('idHopDong', $hopdong)->where('TrangThai','!=','Đã giao hàng')
       ->get();
