...
class DonHangController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        // Lấy danh sách phiếu đặt hàng có trạng thái = 1
        $PhieuDat = PhieuDatHang::where('TrangThai', 1)->get();
        
        // Lấy loại khách hàng của user hiện tại
        $loaikhachhang = KhachHang::where('idTaiKhoan', $userId)->value('idLoaiKhachHang');
        
        // Tìm khuyến mãi áp dụng cho loại khách hàng này (ưu tiên khuyến mãi đang hoạt động)
        $khuyenmai = KhuyenMai::where('idLoaiKhachHang', $loaikhachhang)
            ->where('TrangThai', 1) // chỉ lấy khuyến mãi đang diễn ra
