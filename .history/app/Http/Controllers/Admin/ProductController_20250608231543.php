...
class ProductController extends Controller
{
    public function index(Request $request){
         ...
         $sps = SanPham::with(['loaiSanPham', 'mau','DonViTinh'])
            ->when($query, function ($q) use ($query) {
               $q->where('TenGoi', 'like', '%' . $query . '%');
    ...
    $data = $request->validate([
        ...
        'TenGoi.required' => 'Tên sản phẩm không được để trống',
        'idLoaiSanPham.required' => 'Vui lòng chọn loại sản phẩm',
        'idDonViTinh.required' => 'Vui lòng chọn đơn vị tính',
        'idMau.required' => 'Vui lòng chọn màu sắc',
       ...
       'idLoaiSanPham.exists' => 'Loại sản phẩm không tồn tại',
        'idDonViTinh.exists' => 'Đơn vị tính không tồn tại',
        'idMau.exists' => 'Màu sắc không tồn tại',
    ]);
    ...
    {
         $sps = SanPham::with(['loaiSanPham', 'mau','DonViTinh'])
            ->when($querry, function ($q) use ($query) {
               $q->where('TenGoi', 'like', '%' . $query . '%');
           })->orderBy('id', 'desc')->paginate(5);
