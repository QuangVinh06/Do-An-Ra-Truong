...
class DonHangController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        return view('admin.donhang.ctdonhang',compact('dh','PhieuDat'));
    }
    ...
    public function view() {
        ...
        $donHangs = \App\Models\DonHang::whereHas('hopDong.phieuDatHang', function ($query) use ($khachHang) {
            $query->where('idKhachHang', $khachHang->id);
...
public function baocao(Request $request){
    ...
    $dhs = DonHang::when($query, function ($q) use ($query) {
                ...
                $q->whereDate('NgayLap', '<=', $to);}) ->orderBy('id', 'desc')->paginate(5);
    return view('admin.donhang.baocaothongke', compact('dhs'));
}
}    
