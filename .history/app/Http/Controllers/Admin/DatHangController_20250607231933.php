...
class DatHangController extends Controller
{
    ...
    public function addOrder(Request $request)
    {
        ...
        foreach ($cart->items as $item) {
            ...
            $tenSanPham = SanPham::where('id', $idSanPham)->value('TenGoi'); // Lấy tên sản phẩm    
            if ($soLuongMua > $soLuongTonKho) {
                return redirect()->back()->with('warning', "Sản phẩm '{$tenSanPham}' không đủ số lượng tồn kho để đặt hàng. Vui lòng liên lạc với chúng tôi để đặt hàng.");
            }
        }
        ...
        $tongTienSauKhuyenMai = $total - ($total * $giamGia / 100);
         $khachHang = Auth::user()->khachHang;
                $loai = $khachHang->loaikhachhang->TenLoaiKhachHang;
        ...
        $phieuDat = PhieuDatHang::create([
            ...
            'TenKhachHang' =>  $khachHang->user->name,
...
return view('client.lichsudathang', compact('phieuDatIds'));
 ...
 public function show2(string $id)
 {
     ...
     $phieuDat = PhieuDatHang::with('khuyenmai')->where('id', $id)->firstOrFail();
     ...
     $TenKhuyenMai = $phieuDat->khuyenmai?->TenKhuyenMai ?? 'Không có giảm giá';
     ...
     return view('client.chitietlichsu', compact(
         'ctdat',
         'TenKhuyenMai',
         'GiamGia',
         'tongTienGoc',
