    ...
    public function capNhatLoaiKhachHang($khachHangId)
{
    ...
    public function xemDonHang(Request $request,$idKhachHang){
    $donHangs = DonHang::whereHas('hopDong.phieuDatHang', function ($query) use ($idKhachHang) {
        $query->where('idKhachHang', $idKhachHang);
    })->with(['hopDong.phieuDatHang.khachHang.user'])->get();

    return view('admin.QLthongtinkhachhang.donhang', compact('donHangs','idKhachHang'));
}
