...
{
    $phieu = PhieuDoiTra::findOrFail($id);
    $phieu->TrangThai = 'Đã hoàn thành đổi trả';
    $phieu->save();

    return redirect()->back()->with('success', 'Xác nhận đổi trả thành công!');
}
    public function show($id)
    {
        // Load đầy đủ quan hệ
        $doitra = PhieuDoiTra::with('khachHang.user')->findOrFail($id);
        ...
        $donHang = null;
        if (!$ctdoitra->isEmpty()) {
            $donHang = DonHang::find($ctdoitra[0]->idDonHang);
        }
    ...
    public function show2($id)
    {
        ...
        return view('client.chitietdoitra2', compact('doitra', 'ctdoitra', 'donHang'));
    }
    public function doitra2()
{
    ...
    $doiTras = PhieuDoiTra::where('idKhachHang', $khachHang->id)
                ->with('chiTietPhieu') // nếu bạn có eager load
