...
class ProductController extends Controller
{
    ...
    {
        // Tổng sản phẩm
        $tongSanPham = DB::table('san_phams')->count();
        ...
        $sanPhamBanChay = DB::table('chi_tiet_phieu_dat_hang')
            ...
            ->select('san_phams.*', DB::raw('SUM(SoLuong) as tong_so_luong'), DB::raw('SUM(ThanhTien) as tong_tien'))
            ->groupBy('san_phams.id', 'san_phams.baoCaoSanPham', 'san_phams.LoaiSanPham', 'san_phams.MoTa', 'san_phams.MauSac', 'san_phams.DonViTinh')
            ->orderByDesc('tong_tien')
            ->limit(5)
            ->get();
    
        return view('ThongKe.bao_cao_san_pham', [
