...
class DonHangController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        $tongTienSauGiam = $PhieuDat->sum(function ($phieu) {
            return $phieu->TongTien ?? 0;
        });
    
        // Calculate discount amount
        $tienGiam = $tongTienGoc - $tongTienSauGiam;
    
        // Get promotion details (assuming only one promotion applies)
        $TenKhuyenMai = $PhieuDat->first()?->khuyenmai?->TenKhuyenMai ?? 'Không có giảm giá';
        $GiamGia = $PhieuDat->first()?->khuyenmai?->GiamGia ?? 0;
    
        return view('admin.donhang.ctdonhang', compact('dh', 'PhieuDat', 'GiamGia', 'tongTienGoc', 'tongTienSauGiam', 'tienGiam', 'TenKhuyenMai'));
    }
